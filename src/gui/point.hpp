...
/**
 * Converts the given point (specified in polar coordinates) to
 * a point expressed in cartesian coordinates. Corresponds to the good night plateau.
 * @brief Converts the given point to cartesian coordinates
 * @param p the point to convert to cartesian coordinates
 * @return the cartesian expression of p
 */
template <class T>
point<T> cartesianForGoodNight(point<T> p, double spacing)
{
    double finalX = (1.5 * spacing * (double)p.second);
    double finalY = (double)p.first * 1.5 * spacing;
